...
const Headerpowner = () => {
  ...
  useEffect(() => {
    // Check for low stock (quantity < 100)
    const checkLowStock = () => {
      ...
      fetch(`http://localhost:5000/api/pharmacies/owner/${user.id}`)
        ...
        .then(data => {
          ...
          const low = (data?.stock || []).some(item => typeof item.quantity === "number" && item.quantity < 100);
          setHasLowStock(low);
        })
        .catch(() => setHasLowStock(false));
    };
    checkLowStock();
    window.addEventListener("stock-updated", checkLowStock);
    return () => window.removeEventListener("stock-updated", checkLowStock);
  }, []);
